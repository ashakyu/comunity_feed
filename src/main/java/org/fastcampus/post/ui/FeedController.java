...
public class FeedController {
    ...
    private final UserPostQueueQueryRepository userPostQueueRepository;

    @GetMapping("/{userId}")
    public Response<List<GetPostContentResponseDto>> getPostFeed(@PathVariable(name = "userId") Long userId, Long lastPostId) {
        List<GetPostContentResponseDto> result = userPostQueueRepository.getContentResponse(userId, lastPostId);
